...
#+BEGIN_SRC emacs-lisp
  (setq default-frame-alist
        '((font . "-CTDB-Fira Mono-bold-normal-normal-*-12-*-*-*-m-0-iso10646-1")
...
#+BEGIN_SRC emacs-lisp
  (use-package geiser
    ...
    (defun load-nash ()
      (interactive)
      (unless (get-buffer "* Guile REPL *")
        (run-geiser 'guile))
      (with-current-buffer "* Guile REPL *"
        (geiser-repl--send "(use-modules (nash setup))")))
...
(use-package lispyville
  ...
  :config
  (lispy-define-key lispy-mode-map-special "x" 'lispyville-delete-char-or-splice)
...
*** Utils
#+BEGIN_SRC emacs-lisp
(defun point-to-base (base format)
  (let ((number (string-to-number (word-at-point 'stringp) base)))
    (insert (format format number))))
#+END_SRC

#+BEGIN_SRC emacs-lisp
(defun decimal-to-hex ()
  (interactive)
  (point-to-base 10 "0x%x"))
#+END_SRC

#+BEGIN_SRC emacs-lisp
(defun hex-to-decimal ()
  (interactive)
  (point-to-base 16 "%s"))
#+END_SRC

#+BEGIN_SRC emacs-lisp
(defun binary-to-decimal ()
  (interactive)
  (point-to-base 2 "%s"))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defun http-status-codes ()
    (interactive)
    (find-file-other-window "~/.emacs.d/files/http-status-codes")
    (org-mode)
    (read-only-mode t))

  (defun http-status-codes-find-code ()
     (interactive)
     (http-status-codes)
