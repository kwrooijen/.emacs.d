...
(defun sticky-window-keep-window-visible-frame ()
    "Same as sticky-window-keep-window-visible except that it's not
bound to a single buffer"
  (interactive)
  (push (selected-window) sticky-list))
...
(defun sticky-window-delete-other-windows ()
  ...
  (mapcar (lambda (window)
			(if (and (not (member window sticky-list)) (not (window-dedicated-p window)))
				(delete-window window)))
		  (cdr (window-list))))
